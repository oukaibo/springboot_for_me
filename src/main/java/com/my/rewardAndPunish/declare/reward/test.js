...
function punishUserOnchange(obj) {
    var userCode = $(obj).parent().parent().find("td[data-label='获奖人员工号']").not(".no_data").find('input').val();
    var codes = [];
    ...
    if (pageType != "finishedDetail") {
        ...
        if (activityName == "奖惩申报人提报流程") {
            ...
            for (var i = 0; i < userPunish.length; i++) {
                if ($("[name='table_3pmC']").find("tbody").find("tr").eq(i).find("td[data-label='获奖人员工号']").not(".no_data").find('input').val() != "") {
                    copyUser += $("[name='table_3pmC']").find("tbody").find("tr").eq(i).find("td[data-label='获奖人员工号']").not(".no_data").find('input').val() + ";";
...
function addOnchangeForGetLeader() {
    var userPunish = $("[name='table_3pmC']").find("tbody").find("tr");
    for (var i = 0; i < userPunish.length; i++) {
        var userCode = $("[name='table_3pmC']").find("tbody").find("tr").eq(i).find("td[data-label='获奖人员工号']").not(".no_data").find("input").attr("onchange", "punishUserOnchange(this)");
...
function fileUploadChangeEvent() {
    ...
    for (var i = 0; i < userPunish.length; i++) {
        ...
        var userNo = $("[name='table_3pmC']").find("tbody").find("tr").eq(i).find("td[data-label='获奖人员工号']").find("input").val();
