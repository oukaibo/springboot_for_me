...
$(function() {
	var num = Date.now();
	formUtil.changeHiddenByName("isBelongToStore");
	// setIsBudgetUnlockByActivityName();
	$("[name='mongoKey']").val("AwRBNoIncrementKey");
...
function check_before_submit() {
	...
	if (activityName == '申请人填写') {
		...
		/*if (flag && flag1  && flag3 && flag4   && flag7 && flag8 && flag9 && flag11 && flag12) {
                flag10 = getWerksIsFrozen();
            }*/
		var flag13 = setSummaryByIndex();

		if (subjectType == '09') {
			if (flag && flag1 && flag3 && flag4 && flag8 && flag9 && flag12) {
				//judgeUserIsHeadOrSub();
			...
			if (flag && flag1 && flag3 && flag4 && flag7 && flag8 && flag9 && flag11 && flag12 && flag13) {
				//judgeUserIsHeadOrSub();
				setLinkHandlerBySubclass("[name='bussiness_name']");
				setApprovalPower();
						...
						if (storeCode) {
							for (var i = 0; i < dataArr.length; i++) {
								if (storeCode == dataArr[i].werks) {
									...
									$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").not(".no_data").find(".value_id").val(storeName);
								}
							}
						}
					});
		...
		$.ajax({
			...
			success: function(result) {
				if (result.data.list.length > 0) {
					...
					$("[name='bussiness_name']").attr("data-value", bussinessName);
					common.initSelect();
...
function changeTableProperties() {
	...
	if (activityName == '申请人填写' && (checkBudgetAndAmount == "0" || !checkBudgetAndAmount)) {
		formUtil.changeEditByName("subject_type");
		formUtil.changeEditByName("bussiness_name");
	}
...
function addOnChangeEvent() {
	...
	$("[name='writeOffLoanTable'] tbody").find("tr").find("td[data-label='本次冲销金额']").find("input").attr("onchange", "getRestMoney();changeByIsRevertBorrow();checkAndfomartNumber(this,16,2);");
	$("[name='writeOffLoanTable'] tbody").find("tr").find("td[data-label='归还金额']").find("input").attr("onchange", "judgeRevertIsTrue(this);getRestMoney();checkAndfomartNumber(this,16,2);showOrHideMustByRepayment(this);");
	$("[name = 'reimburseDetail'] tbody td[data-label='人民币金额']").find("input").attr("onchange", "checkAndfomartNumber(this,16,2);getAmount();judgermbAmountIsExceed(this);giveMoney(this);getTaxesByAmountAndTaxRate(this);");
	$("[name = 'reimburseDetail'] tbody td[data-label='金额']").find("input").attr("onchange", "checkAndfomartNumber(this,16,2);getRMBAmunt(this);");
	$("[name = 'reimburseDetail'] tbody td[data-label='汇率']").find("input").attr("onchange", "checkAndfomartNumber(this,16,2);getRMBAmunt(this);");
	//$("[name = 'reimburseDetail'] tbody td[data-label='(供应商/个人)编码']").find("input").attr("onchange", "compareReceiveWithDetails()");
	$("[name = 'travelInfo'] tbody td[data-label='报销金额']").find("input").attr("onchange", "getAmount();changeByIsRevertBorrow();checkAndfomartNumber(this,16,2);giveMoney(this);getTaxesByAmountAndTaxRate(this);");
	...
	$("[name = 'reimburseDetail'] tbody td[data-label='收款人类型']").find("select").attr("onchange", "changeCostAttribute(this);changeSubsidyDateShowOrHide();setPropertiesByType(this);changeByCostAttribute(this);changePayeesHideOrShow();");
	...
	$("[name = 'travelInfo'] tbody td[data-label='工号']").find("input[type='text']").attr("onchange", "getSysAwBankInfo(this);");
	...
	$("[name = 'travelInfo'] tbody td[data-label='备注']").find("select").attr("onchange", "setNullRemarkInTravelInfo(this);");
...
function changeByIsChargeAgainst(obj) {
	...
	switch (isChargeAgainst) {
		...
		case '1':
			...
			$("[name='reimburseDetail'] tbody").find("tr:eq(0)").find("td[data-label='币种']").not(".no_data").find("select").val("CNY");
			$("[name='reimburseDetail'] tbody").find("tr:eq(0)").find("td[data-label='汇率']").not(".no_data").find("select").val(1);
			formUtil.tableFun.changeEditByTableParam({
				name: 'reimburseDetail',
				colNum: 3
			});
...
function checkSubjectCode() {
	...
	if (rowList.length > 1) {
		for (var i = 0; i < rowList.length; i++) {
			...
			var costCodeFirst = $(rowList[i]).find("td[data-label='(供应商/个人)编码']").find("input").val();
						...
						if (loanObj.id == detailuid) {
							...
							if (restAmount >= 0) {
								...
								for (var k = 0; k < rowList.length; k++) {
									...
									var amount = parseFloat($(rowList[k]).find("td[data-label='人民币金额']").find("input").val());
									if (loanNumberInRB == number) {
										total += amount;
									}
								}
								$(loanList[i]).find("td[data-label='本次冲销金额']").find("input").val(total);
								checkAndfomartNumber($(loanList[i]).find("td[data-label='本次冲销金额']").find("input"), 16, 2);
								getRestMoney();
								...
								$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='人民币金额']").find("input").val(loanObj.residualAmount);
								...
								$("[name='reimburseDetail'] tbody").find("tr").each(function() {
									...
									var rest = parseFloat($(this).find("td[data-label='人民币金额']").find("input").val());
...
function getRMBAmunt(obj) {
	var index = $(obj).parent().parent().index();
	var amount = $("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='金额']").not(".no_data").find("input").val();
	var currency = $("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='币种']").not(".no_data").find("select").val();
	amount = isNaN(parseFloat(amount)) ? 0 : amount;
	var exchangeRate = "";
	if (!currency || currency == 'CNY') {
		exchangeRate = 1.00;
		$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='汇率']").not(".no_data").find("input").val(exchangeRate);
	} else {
		exchangeRate = $("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='汇率']").not(".no_data").find("input").val();
	}

	exchangeRate = isNaN(parseFloat(exchangeRate)) ? 0 : exchangeRate;

	var rmbAmount = NumUtil.multiply(amount, exchangeRate, 2);
	$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='人民币金额']").not(".no_data").find("input").val(rmbAmount);
	$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='人民币金额']").not(".no_data").find("input").change();
	...
	if (isChargeAgainst == '0') {
		...
		if (getValType == '09') {
			...
			if (total > 0) {
				for (var j = 0; j < writeOffLoanList.length; j++) {
					...
					checkAndfomartNumber($(writeOffLoanList[j]).find("td[data-label='本次冲销金额']").find("input"), 16, 2);
				}
			...
			for (var i = 0; i < amountList.length; i++) {
				var price = parseFloat($(amountList[i]).find("td[data-label='人民币金额']").find("input").val());
		...
		if (getValType == '09') {
			...
			for (var i = 0; i <= amountList.length - 1; i++) {
				var price = $("[name = 'reimburseDetail'] tbody").find("tr").eq(i).find("td[data-label='人民币金额']").find("input").val();
				...
				price = isNaN(price) ? 0 : price;
				if (price == 0) {
					$("[name='reimburseDetail'] tbody").find("tr:eq(" + i + ")").find("td[data-label='人民币金额']").not(".no_data").find("input").val(price);
					checkAndfomartNumber($("[name='reimburseDetail'] tbody").find("tr:eq(" + i + ")").find("td[data-label='人民币金额']").find("input"), 16, 2);
				}
				taxesAmount = NumUtil.add(taxesAmount, taxes, 2);
				total = NumUtil.add(total, price, 2);
			}
...
function changeCostAttribute(obj) {
	...
	if (costAttribute == "3" || costAttribute == "6") {
		$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find("input[type='text']").val(userId);
		if (!storeCode) {
			$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find(".value_id").val(userId);
		}
		$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find(".value_code").val(userId);
		...
		$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find("input[type='text']").val("9903000");
		if (!storeCode) {
			$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find(".value_id").val("9903000");
		}
		$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find(".value_code").val("9903000");
		formUtil.tableFun.changeNotEditByTableParam({
			name: 'reimburseDetail',
			colNum: 13,
			rowNum: index
		...
		$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find("input").val("");
	...
	addOnChange(obj);
	var attribute = $("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find("input").val();
	if (attribute) {
		$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find("input[type='text']").change();
	}
...
function changeEditByBussiness(obj) {
	...
	var activityArr = ["爱屋财务会计", "爱屋财务经理", "推送报销系统"];
	if (bussinessName == 'WZCG05' || bussinessName == 'WZCG06') {
		formUtil.tableFun.chooseModelHiddenByTableParam({
			name: 'reimburseDetail',
			colNum: 6
...
common.chooseDicDataByTypePath = function(id, dicUid, dicCode) {
	return common.getPath() + "/LYFData/selectDataByRB?elementId=" + id + "&dicUid=" + dicUid + "&dicCode=AW";
...
function rowRemoveChangeEvent(obj) {
	...
	if (name == "reimburseDetail") {
		$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='人民币金额']").find("input").val(0);
		$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='人民币金额']").find("input").change();
...
function setReimbursementAccount() {
	...
	if (writeOffLoan == '0' && payment < 0) {
		layer.msg("冲销额外报销时,请保证实际报销金额不低于0,请重新确认", {
			icon: 2
		});
		return false;
	}
...
function setChooseInvoiceShowOrHide() {
	...
	var total = 0;
	var activityArr = ["爱屋财务会计", "爱屋财务经理", "推送报销系统"];
	var hiddenMustArr = ["部门经理1", "部门经理2", "互联网财经部经理", "项目总监", "项目经理", "互联网事业群副总", "爱屋总裁审批", "职能联签(人力)", "职能联签(法务)", "职能联签(行政)", "总裁审批"];
	for (var i = 0; i < rowList.length; i++) {
...
function changeColumnShowOrHide() {
	...
	var activityName = $("#activityName").val();
	$("[name='reimburseDetail'] tbody").find("tr").each(function() {
		var currency = $(this).find("td[data-label='币种']").not(".no_data").find("select").val();
		var exchangeRate = $(this).find("td[data-label='汇率']").not(".no_data").find("input").val();
		if (!currency) {
			$(this).find("td[data-label='币种']").not(".no_data").find("select").val("CNY");
		}
		if (!exchangeRate) {
			$(this).find("td[data-label='汇率']").not(".no_data").find("input").val("1");
		}
	});
	// for (var i = 0; i < dataList.length; i++) {
...
function getLinInfoByCode(obj) {
	...
	var zhuanzhangMoney = $(obj).parent().parent().find("td[data-label='人民币金额']").not(".no_data").find('input').val();
	var remark = $(obj).parent().parent().find("td[data-label='备注']").find("input").val();
	remark = remark.substring(0, 20);
	var code = $(obj).parent().parent().find("td[data-label='(供应商/个人)编码']").not(".no_data").find("input[type='text']").val().trim();
	...
	if (code != "") {
		...
		$.ajax({
			...
			success: function(result) {
				...
				$(obj).parent().parent().find("td[data-label='(供应商/个人)编码']").not(".no_data").find('.value_id').val(storeName);
...
function getSysAwBankInfo(obj) {
	...
	if (tableName == "reimburseDetail") {
		...
		amount = $(obj).parent().parent().find("td[data-label='人民币金额']").not(".no_data").find('input').val();
		code = $(obj).parent().parent().find("td[data-label='(供应商/个人)编码']").not(".no_data").find("[type='text']").val().trim();
		...
		remark = "市外差旅费";
		shoukuanrentype = "4";
		amount = $(obj).parent().parent().find("td[data-label='报销金额']").not(".no_data").find('input').val();
	...
	if (code != "") {
		$.ajax({
			url: common.getPath() + '/sysAwBankinfo/searchSysAwBankinfo?receiveCode=' + code + "&receiveType=3",
			type: 'post',
			contentType: 'application/json;charset=UTF-8',
			success: function(result) {
				...
				if (result.status == 0) {
					if (!data) {
						...
						if (subjectType == '09') {
							$("[name='travelInfo'] tbody").find("tr:eq(" + index + ")").find("td[data-label='工号']").find("input").val("");
						} else {
							$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find("input").val("");
							$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='名称']").find("input").val("");
						}
					...
					if (subjectType == '09') {
						$("[name='travelInfo'] tbody").find("tr:eq(" + index + ")").find("td[data-label='工号']").find("input").val(data.receiveCode);
						$("[name='travelInfo'] tbody").find("tr:eq(" + index + ")").find("td[data-label='工号']").find(".value_id").val(data.receiveName);
					} else {
						$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find("input").val(data.receiveCode);
						$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find(".value_id").val(data.receiveName);
						$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='名称']").find("input").val(data.receiveName);
					}
					$("[name='payees'] tbody").find("tr").each(function() {
...
//费用明细选择收款人类型添加onchange事件
function addOnChange(obj) {
	var num = Date.now();
	var shoukuanrentype = $(obj).val();
	var index = $(obj).parent().parent().index();
	var lifnrCode = $(obj).parent().parent().find("td[data-label='(供应商/个人)编码']").not(".no_data").find("input[type='text']").val();
	var detailId = $(obj).parent().parent().find("td[data-label='排序']").not(".no_data").find('input').val();
	switch (shoukuanrentype) {
		case '1':
			$("[name = 'reimburseDetail'] tbody ").find("tr").eq(index).find("td[data-label='(供应商/个人)编码']").not(".no_data").find("input[type='text']").attr("onchange", "getWerkAndLinInfo(this);");
			formUtil.tableFun.changeEditByTableParam({
				name: 'reimburseDetail',
				colNum: 13,
				rowNum: index
			...
			break;
		case '2':
			$("[name = 'reimburseDetail'] tbody ").find("tr").eq(index).find("td[data-label='(供应商/个人)编码']").not(".no_data").find("input[type='text']").attr("onchange", "getLinInfoByCode(this)");
			formUtil.tableFun.changeEditByTableParam({
				name: 'reimburseDetail',
				colNum: 13,
				rowNum: index
			...
			break;
		case '3':
			$("[name = 'reimburseDetail'] tbody ").find("tr").eq(index).find("td[data-label='(供应商/个人)编码']").not(".no_data").find("input[type='text']").attr("onchange", "getSysAwBankInfo(this);");
			formUtil.tableFun.changeNotEditByTableParam({
				name: 'reimburseDetail',
				colNum: 13,
				rowNum: index
		...
		case '5':
			...
			break;
		default:
			$("[name = 'reimburseDetail'] tbody ").find("tr").eq(index).find("td[data-label='(供应商/个人)编码']").not(".no_data").find("input[type='text']").attr("onchange", "");
...
function giveMoney(obj) {
	...
	if (($("[name='writeOffLoan']").val() == "0") && ($("[name='subject_type']").val() == "09")) {
		...
		if (subjectType != "09") {
			jieKuanMoney = $("[name = 'reimburseDetail'] tbody ").find("tr").eq(rowIndex).find("td[data-label='人民币金额']").not(".no_data").find("input").val();
...
//收款人类型为供应商、门店、一����性供应商时默认成本中心为发起人的成本中心，名称是发起人的部门名称
                ...
                success: function(result) {
                    if (result != "") {
                        ...
                        if (tableName == 'reimburseDetail') {
                            ...
                            if (!storeCode) {
                                $("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find(".value_id").val(result.userName);
                            }
                            $("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find(".value_code").val(userUid);
                        } else
                        ...
                        if (tableName == 'travelInfo') {
                            ...
                            $("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").not(".no_data").find("input[type='text']").val("");
                            $("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").not(".no_data").find(".value_code").val("");
                        }
                        return;
                    }
                },
	...
	if (successList.length > 0) {
		...
		if ($("[name='subject_type']").val().trim() == "09") {
			...
			for (var ri = 0; ri < TrLengthAfterAddRows; ri++) {
				...
				$("[name='reimburseDetail'] tbody").find("tr:eq('" + ri + "')").find("td[data-label='人民币金额']").find("input").change();
...
function getTaxesByAmountAndTaxRate(obj) {
	...
	var amount = 0;
	if (tableName == 'reimburseDetail') {
		amount = parseFloat($("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='人民币金额']").find("input").val());
	} else if (tableName == 'travelInfo') {
...
function changeSubsidyDateShowOrHide() {
	...
	if (bussinessName == 'MDRC08') {/*formUtil.tableFun.chooseModelShowByTableParam({
                name: 'reimburseDetail',
                colNum: 35
            });
            formUtil.tableFun.chooseModelShowByTableParam({
	...
	} else {/*formUtil.tableFun.chooseModelHiddenByTableParam({
                name: 'reimburseDetail',
                colNum: 35
            });
            formUtil.tableFun.chooseModelHiddenByTableParam({
	...
	if (subjectType == '09' && writeOffLoan == '1') {
		...
		$("[name='reimburseDetail'] tbody").find("tr").each(function() {
			...
			$("[name='reimburseDetail'] tbody").find("tr").each(function() {
				...
				var total = parseFloat($(this).find("td[data-label='人民币金额']").find("input").val());
...
common.chooseDicDataPath = function(id, dicUid, dicCode) {
	...
	var costAttribute = $("#" + id).parent().parent().find("td[data-label='收款人类型']").find("select").val();
	var bussinessName = $("[name='bussiness_name']").val();
	...
	if (tdName == '科目编码') {
		...
		if (costAttribute == "4" || costAttribute == undefined || bussinessName == 'SWCL01') {
...
common.chooseDicData = function(id, dicUid, dicCode) {
	...
	var costAttribute = $("#" + id).parent().parent().find("td[data-label='收款人类型']").find("select").val();
	var subjectType = $("[name='subject_type']").val();
	// if (!costAttribute && subjectType != '09') {
	//     layer.alert("请先选择收款人类型！");
	//     return;
	// }
	var subjectType = $("[name='subject_type']").val();
	if (subjectType != '09' && costAttribute == '2') {
		return common.chooseSysAwLifnr(id, costAttribute);
	}
	var title = "";
	if (tdName == '(供应商/个人)编码') {
		title = $("#" + id).parent().parent().find("td[data-label='收款人类型']").find("select").find("option:selected").text() + "信息";
		if (title == '他人信息') {
			title = "员工信息";
		}
	} else if (tdName == '科目编码') {
...
common.chooseDepart = function(elementId) {
	...
	if (!company) {
		layer.alert("请先选择报销所属公司，再选择成本中心");
		return;
	}
				...
				$("[name='reimburseDetail'] tbody").find("tr").each(function() {
					...
					for (var i = 0; i < result.data.length; i++) {
						if (reimbMonth == result.data[i].month) {
							$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='车贴金额']").not(".no_data").find("input").val(result.data[i].canUserQuota);
						}
					}
				});
			}
	...
	if (code && transSubsidyDate) {
		$.ajax({
			...
			success: function(result) {
				...
				$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='车贴金额']").not(".no_data").find("input").val(canUserQuota);
...
function setPropertiesByType(obj) {
	...
	$("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find("input[type='text']").removeAttr("readonly");
...
function queryStoreLastMonthDetialForUpload() {
	...
	if (subjectType == "07" && bussinessName == "MDRC08") {
		...
		for (var i = 0; i < detailTrs.length; i++) {
			var storeCode = $("[name='reimburseDetail']").find("tbody").find("tr").eq(i).find("td[data-label='(供应商/个人)编码']").find("input[type=text]").val();
...
function getPreLiftInfo(obj) {
	...
	var index = $(obj).parent().parent().index();
	var tableName = $(obj).parent().parent().parent().parent().attr("name");
	var lifnr = $("[name='" + tableName + "'] tbody").find("tr:eq(" + index + ")").find("td[data-label='(供应商/个人)编码']").find("input").val();
	var companyNum = $("#companyNum").val();
	if (lifnr) {
...
function judgeTotalEqualTaxAndOrderMoney() {
	...
	if (orderMoney && isPurchaseOrder == '0') {
		$("[name='reimburseDetail'] tbody").find("tr").each(function() {
			var amount = parseFloat($(this).find("td[data-label='人民币金额']").find("input").val());
	...
	$("[name='reimburseDetail'] tbody").find("tr").each(function() {
		...
		switch (receiveType) {
			...
			case '2':
				...
				break;
			case '3':
				$("[name = 'reimburseDetail'] tbody ").find("tr").eq(index).find("td[data-label='(供应商/个人)编码']").not(".no_data").find("input[type='text']").attr("onchange", "obtainBankInformationAccordingToWorkNumber(this);");
				formUtil.tableFun.changeNotEditByTableParam({
					name: 'reimburseDetail',
					colNum: 13,
					rowNum: index
...
function setSummaryByIndex() {
	...
	$("[name='reimburseDetail'] tbody").find("tr").each(function() {
		...
		var storeName = "";
		if (storeCode) {
			storeName = $(this).find("td[data-label='(供应商/个人)编码']").find(".value_id").val();
		}
...
function judgeAmountIsOverCheckAmount() {
	...
	if (subjectType == '09') {
		...
		$("[name='reimburseDetail'] tbody").find("tr").each(function() {
			...
			var index = $(this).index();
			if (amount != checkAmount) {
				$(this).find("td[data-label='人民币金额']").not(".no_data").find("input").val(checkAmount);
				giveMoney($("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='人民币金额']").find("input"));
				getTaxesByAmountAndTaxRate($("[name='reimburseDetail'] tbody").find("tr:eq(" + index + ")").find("td[data-label='人民币金额']").find("input"));
			}
		});
	}
	getAmount();
	...
	if (activityName == '申请人填写') {
		...
		if (subjectType == '09') {
			$("[name='travelInfo'] tbody").find("tr").each(function() {
				...
				if (!checkAmount || checkBudgetAndAmount == '0' || !checkBudgetAndAmount) {
			...
			$("[name='reimburseDetail'] tbody").find("tr").each(function() {
				var amount = $(this).find("td[data-label='人民币金额']").find("input").val();
				var checkAmount = $(this).find("td[data-label='验证金']").find("input").val();
				if (!checkAmount || checkBudgetAndAmount == '0' || !checkBudgetAndAmount) {
					$(this).find("td[data-label='验证金']").not(".no_data").find("input").val(amount);
				}
				...
				$("[name='reimburseDetail'] tbody").find("tr").each(function() {
					...
					for (var i = 0; i < dataArr.length; i++) {
						if (dataArr[i].werks == code) {
							...
							$(this).find("td[data-label='(供应商/个人)编码']").not(".no_data").find(".value_id").val(storeName);
						}
					}
...
function NumberFormat() {
	...
	if (subjectType == '09') {
		...
		$("[name='reimburseDetail'] tbody").find("tr").each(function() {
			checkAndfomartNumber($(this).find("td[data-label='人民币金额']").find("input"), 16, 2);
			var taxes = $(this).find("td[data-label='税金']").find("input").val();
			if (taxes) {
				checkAndfomartNumber($(this).find("td[data-label='税金']").find("input"), 16, 2);
			}
...
// 申请环节提供验证供应商 若收款人对应供应商时且收款人为9903000的情况下是否出现多条
function judgePayeeTypeIsTrue() {
	...
	$("[name='payees'] tbody").find("tr").each(function() {
		...
		if (payeeType == '2' && payeeCode == '9903000') {
